...
def main():
    # A. McKay
    # initialize model 
    image_paths1 = get_image_paths('../data/bed')
...
def save_image(results_image_paths):
    # From G. Surma
    print(f'Result image saved at {COMBINED_IMAGE_PATH}')
    combined = Image.new("RGB", (IMAGE_WIDTH*len(results_image_paths), IMAGE_HEIGHT))
    x_offset = 0
    for image in map(Image.open, results_image_paths):
